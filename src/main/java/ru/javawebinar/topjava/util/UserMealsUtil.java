...
public class UserMealsUtil {
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals,
        ...
        for (UserMeal meal : meals) {
            ...
            int total = totalCaloriesPerDay.getOrDefault(mealDate, 0) + meal.getCalories();
            totalCaloriesPerDay.put(mealDate, total);
        }

        List<UserMealWithExcess> mealWithExcesses = new ArrayList<>();
        for (UserMeal meal : meals) {
            LocalDateTime mealDateTime = meal.getDateTime();
            int total = totalCaloriesPerDay.get(mealDateTime.toLocalDate());
