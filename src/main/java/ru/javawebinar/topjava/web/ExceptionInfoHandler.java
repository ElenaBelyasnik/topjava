...
public class ExceptionInfoHandler {
    ...
    public ErrorInfo bindValidationError(HttpServletRequest request, BindException error){
        String[] details = error.getBindingResult().getFieldErrors().stream()
                .map(fieldError -> String.format("[%s] %s", fieldError.getField(), fieldError.getDefaultMessage()))
                .toArray(String[]::new);

        return logAndGetErrorInfo(request, error, false,VALIDATION_ERROR,details);
    }
