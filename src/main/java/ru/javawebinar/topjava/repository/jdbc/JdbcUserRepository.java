...
//        return jdbcTemplate.queryForObject("SELECT * FROM users WHERE email=?", ROW_MAPPER, email);
    ...
    public List<User> getAll() {
        ...
        jdbcTemplate.query("SELECT * FROM user_role", resultSet -> {
            map.computeIfAbsent(resultSet.getInt("user_id"),
                    userId -> EnumSet.noneOf(Role.class)).add(Role.valueOf(resultSet.getString("role")));
        });
    ...
    private User setRoles(User u) {
        if (u != null) {
            List<Role> roles = jdbcTemplate.queryForList("SELECT user_role.role FROM user_role WHERE  user_id = ?", Role.class, u.getId());
            u.setRoles(roles);
        }
        return u;
    }
